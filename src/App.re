       ...
       <li
         ...
         <div className="px-6 py-4">
           ...
           <p className="font-mono text-pink-lighter text-xs">
             {repo.description |> renderDesc |> React.string}
           </p>
         </div>
...
let initialState = {username: "", user: "", repos: RemoteData.NotAsked};
let reducer = (state, action) =>
  ...
  | ReposLoaded(repos) => {...state, repos: RemoteData.Success(repos)}
  | ReposError(err) => {...state, repos: RemoteData.Failure(err)}
  };
         ...
         {if (List.length(repos) > 0) {
            <div
              ...
              <ul
                ...
                {React.array(repos |> repoItems(state.user) |> Array.of_list)}
              </ul>
            </div>;
          } else if (!isLoading) {
            <p className="mt-8 font-mono text-pink text-lg">
